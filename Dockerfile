...
COPY . /home/developer/go/src/github.com/scipie28/note-service-api
RUN echo developer | sudo -S chown -R developer /home/developer/

WORKDIR /home/developer/go/src/github.com/scipie28/note-service-api

RUN make deps && make vendor-proto && make generate && make build

FROM alpine:latest
RUN apk --no-cache add ca-certificates
WORKDIR /root/
COPY --from=builder /home/developer/go/src/github.com/scipie28/note-service-api/bin/note-service-api .
RUN chown root:root note-service-api
EXPOSE 8080
CMD ["./note-service-api"]